...
function acceso (credencial : string, hora: number): boolean {
    let salida: boolean = false;
    if (credencial == "Estandar") {
        if ((hora >=100 && hora <=1200)) {salida = true;}
        console.log ("Estandar: ", salida);
        return (salida);
    }
    if (credencial == "Plata") {
        if ((hora >=800 && hora <=1500)) {salida = true;}
        console.log ("Plata: ", salida);
        return (salida);
    }
    if (credencial == "Oro") {
        if ((hora >=800 && hora <=1800)) {salida = true;}
...
//Pregunta 1.b
let T: string [] = ["Oro", "Plata", "Estandar", "Platino"];
let i: number = 0;
let horaactual: number = 1330;
let suma: number = 0;
